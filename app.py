...
from pod_ops import create_custom_resource_object, get_pods_info, delete_custom_resource_object, get_clusters_info, update_cluster
import config
...
def create_cluster():
    ...
    logging.info(
        "creating custom resource object for %s cluster " %
        cluster_name)
...
def scaleup_scaledown():
    ...
    cluster_name = cluster_data['cluster_name']
    name_space = cluster_data['name_space']
    rack_members = int(cluster_data['rack_members'])
    operation = cluster_data['operation']
    CRD_PATH = os.path.join(cluster_name, 'crd.json')
